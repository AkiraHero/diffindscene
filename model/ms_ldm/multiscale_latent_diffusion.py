...
class MSLDM(ModelBase):
    ...
    def __init__(self, config) -> None:
        ...
        if self.use_sketch_condition:
            ...
            smodel.load_model_paras_from_file(config.paras.sketch_embedder.ckpt)
            smodel = smodel.eval()
    ...
    def init_level(self, level, mode="train"):
        ...
        if mode == "test":
            if "ckpt" in self.model_paras[self.model_level]:
                ckpt_name = self.model_paras[self.model_level]["ckpt"]
                ckpt_content = torch.load(ckpt_name)
                pop_keys = []
                ...
                logging.info("[LDM]load model:{}".format(ckpt_name))
    ...
    def init_first_stage_module(self, config):
        ...
        model.load_model_paras_from_file(config.ckpt)
        model = model.eval()
        return model
